...
判断基準
- 実行系は Cursor に依頼しない（実行は GPT の PowerShell コマンド提示 + ユーザー貼り付け実行）。
- コメントアウト付き実行文、EOFっぽい崩れ、別文脈混入、聞き返し増加が出たら、その実行は破棄して新チャット候補にする。
- 新チャット移行はターン数ではなく、上記症状の発生有無で判断する。

注意点
- 軽量運用（`status.md` / `handoff.md` 中心）は維持する。
- `PROJECT_STATE.md` / `rules.md` は必要時のみ参照する。
- engine 本体と `stability-test.ps1` は今回変更しない。
