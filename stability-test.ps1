...
function Ensure-ChildHelperFunctions {
  ...
  if (-not (Get-Command Invoke-RecoveredPostNavigateExecuteProbe -ErrorAction SilentlyContinue)) {
    function script:Invoke-RecoveredPostNavigateExecuteProbe([string]$label, [string]$scriptText) {
      $scriptLabel = "recovered-post-nav-" + $label
      $payload = @{ script = $scriptText; args = @() }
      $requestUrl = "$($script:driverBaseUrl)/session/$($script:sessionId)/execute/sync"
      $bodyJson = $payload | ConvertTo-Json -Depth 8
