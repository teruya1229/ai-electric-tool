...
function parserTraceIndicatesCompatAttentionForDisplay(parser) {
  ...
  if (parser.source === "meta") {
    return (
      parser.canContinue === true &&
      (String(parser.severity || "") === "compat-warning" ||
        String(parser.reasonCode || "").startsWith("compat.") ||
        parser.metaHasCompatError === true ||
        parser.metaHasCompatWarning === true)
    );
  }
  if (parser.source === "text-fallback") {
    return String(parser.reasonCode || "") === "compat.error-allow-continue";
  }
  return false;
}
...
function resolveFinalRenderDecision({ shouldRender, parser, diagramSummary }) {
  ...
  const displayWarning =
    parserTraceIndicatesCompatAttentionForDisplay(parser) ||
...
function formatDiagramReasonCodesUserHint(diagramReasonCodes) {
  ...
  if (diagramReasonCodes.some((c) => String(c).includes("single_3switches_1light_diagram_one"))) {
    hints.push("片切スイッチ3個 + 照明1灯は、図では代表形にまとめて表示しています");
  }
  if (
  ...
  if (diagramReasonCodes.some((c) => String(c).includes("threeway_4lights_diagram_one"))) {
    hints.push("3路で4灯の系統は、図では1灯としてまとめて示しています。");
  }
  return hints.join(" ");
}
