...
## 2026-05-25 片切系MVP完成・現場運用フェーズ移行
...
- 対応済みの主な範囲:
  ...
  - 片切系の複数コンセント partial
  - 照明3灯以上は2灯代表図へ寄せる方針
  - コンセント複数は1個代表表示 + 補助情報扱いの方針
- 完全な多灯描画には進めず、代表表示で運用する。
- 完全な複数コンセント描画には進めず、代表表示で運用する。
- `reasonCodes: n/a` は既知差分として今は追わない。
- 3路+コンセントは parser 側エラー停止があり、別フェーズ扱い。
- `PROJECT_STATE.md` 更新や未追跡ファイル整理は別タスク扱い。

### 今後の運用方針
- 今後は現場で実際に使いながら改善する。
- これ以上、灯数や細かい入力パターンを事前に無限追加しない。
- 現場で実際に困った入力だけ、再現文・期待値・実際の表示を記録して改善対象にする。
- 新規改善は1ケースずつ、`js/diagram/index.js` の1ファイル最小差分を基本にする。
- parser / UI / warning / `wiring-diagram.js` に広がる場合は、その場で実装せず別タスク化する。

## 2026-05-25 片切4 + 照明6灯同時点灯 representative compatibility 完了
...
- DOM上で `single_4switches_6lights_diagram_two` が表示されない件も、reasonCode表示未接続の既知差分として今回は追わない。
...
### 主な到達コミット
- `a676541`: `single_4switches_6lights_diagram_two` を追加した実装コミット

### 現在の状態
- 片切4 + 6灯同時点灯は限定対応として完了扱い。
- 片切1 + 6灯同時点灯、片切2 + 6灯同時点灯、片切3 + 6灯同時点灯も限定対応として完了扱い。
- 片切1/2/3/4 + 6灯同時点灯が一通り代表表示で揃った。
- 片切1/2/3/4 + 5灯同時点灯も限定対応として完了扱い。
- ただし、5灯/6灯全体解禁ではない。
...
## 2026-05-25 片切3 + 照明6灯同時点灯 representative compatibility 完了
...
### 主な到達コミット
- `bb44ee8`: `single_3switches_6lights_diagram_two` を追加した実装コミット

### 現在の状態
- 片切3 + 6灯同時点灯は限定対応として完了扱い。
- 片切1 + 6灯同時点灯、片切2 + 6灯同時点灯も限定対応として完了扱い。
